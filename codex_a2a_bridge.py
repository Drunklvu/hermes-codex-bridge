...
# 监控页 UI 模板（独立文件，拆自 MONITOR_UI_HTML，2026-08-08）
# 零依赖读取（Path.read_text 纯 stdlib）；文件缺失时报清晰错误
_UI_TEMPLATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "monitor_ui.html")
try:
    with open(_UI_TEMPLATE_FILE, encoding="utf-8") as _f:
        MONITOR_UI_HTML = _f.read()
except FileNotFoundError:
    MONITOR_UI_HTML = (
        "<!DOCTYPE html><html><head><title>UI template missing</title></head>"
        "<body><h1>monitor_ui.html 缺失</h1><p>请确认该文件与 codex_a2a_bridge.py 同目录。</p></body></html>"
    )
    logging.warning("monitor_ui.html 未找到（%s），监控页降级为错误提示", _UI_TEMPLATE_FILE)
